"""
Data contracts for pandas DataFrames.
...
Use case 1 — pure data contracts (no lock file needed):
    ...
    safe_fn = spec.wrap(transform)       # returns Verified[DataFrame]
    result = safe_fn(df)                 # result.inner is the DataFrame
...
Use case 2 — version-guarded contracts:
    ...
    # Fails at construction time if pandas < 3.0.0 in lock file

Use case 3 — safe/unsafe boundary (Rust-inspired):
    verified_df = spec.verify(raw_df)    # safe: contracts checked
    raw = Verified.unsafe(raw_df)        # unsafe: explicitly bypassed
    # Both are Verified[DataFrame], but .contracts tells you which path
"""
...
def verified(
    ...
    *lock_versions* to also validate dependency versions at decoration time.
    ...
    require : dict[str, str] | None
        Package version constraints (e.g. {"pandas": ">=3.0.0"}).
